...
/**
 * Class CompileScalabilityTemplate
 * @package emPHPasis\Pipelines\Pipes\Generate
 */
class CompileScalabilityTemplate extends Pipe
{
            ...
            if ($code == $passable::SUCCESS_CODE) {
                ...
                if (file_exists($templatePath)) {
                    ...
                    $indexHtml = $pugEngine->renderFile(
                        $templatePath . 'scalability.pug',
                        $data
                    );
                    $this->writeFile($reportPath . 'scalability.html', $indexHtml);

                    // write to output
                    $passable->getOutputInterface()
                        ->writeln(
                            Carbon::now() . ' Compiled testability template: ' . $templatePath . 'scalability.html'
                        );
