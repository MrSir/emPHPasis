...
/**
 * Class CompileComplexityTemplate
 * @package emPHPasis\Pipelines\Pipes\Generate
 */
class CompileComplexityTemplate extends Pipe
{
            ...
            if ($code == $passable::SUCCESS_CODE) {
                ...
                if (file_exists($templatePath)) {
                    ...
                    $indexHtml = $pugEngine->renderFile(
                        $templatePath . 'complexity.pug',
                        $data
                    );
                    $this->writeFile($reportPath . 'complexity.html', $indexHtml);

                    // write to output
                    $passable->getOutputInterface()
                        ->writeln(
                            Carbon::now() . ' Compiled testability template: ' . $templatePath . 'complexity.html'
                        );
